...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4092289,40482735)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4092289,40482735)
...
-- End Criteria Group
  ...
  WHERE DATEADD(day,0,OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE AND DATEADD(day,37,E.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
) P
WHERE P.ordinal = 1
-- End Primary Events
) pe
...
WHERE YEAR(E.start_date) - P.year_of_birth <= 1
GROUP BY e.person_id, e.event_id
...
WHERE YEAR(C.start_date) - P.year_of_birth >= 5
